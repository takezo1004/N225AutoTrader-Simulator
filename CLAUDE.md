...
> このファイルは、**基本ルート（[`README.md`](README.md) の setup.exe 導入＆起動）でうまくいかなかったとき**に、
> 購読者の Claude Code が手伝うための命令書です。基本が動いていれば本ファイルは不要です。
>
> v0.5.0（2026-07-14）：ブリッジは **setup.exe（実行ファイル）のみ**の同梱になりました（ソース非同梱）。
> 旧 v0.4.x の「bridge/ ソース同梱・dotnet build」前提は廃止です。
...
- テストダッシュボード（Python・stdlib のみ・ソース公開）が導入済みブリッジを自動検出して `--simulator` 起動＋7種ペイロード発火。
...
3. **環境差を吸収**：先に `/diagnose` → 不足（Python）を順次解決。.NET は不要。
...
| コマンド | 用途 |
|---|---|
| `/install` | 環境構築（同梱 **setup.exe でブリッジ導入** → Python 確認） |
