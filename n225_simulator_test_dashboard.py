...
BRIDGE_EXE_NAME = "N225BrokerBridge.UI.exe"
# setup.exe (Inno) の AppId。アンインストールキーは "<AppId>_is1"（installer/N225BrokerBridge.iss と一致させる）
BRIDGE_INSTALLER_APP_ID = "{A0B1C2D3-E4F5-6789-ABCD-EF0123456789}"


def _installed_bridge_locations() -> list[Path]:
    """setup.exe で導入済みのブリッジを探す（配布環境の本命）。
    ① レジストリのアンインストールキー InstallLocation（既定以外の導入先に対応）
    ② 既定の導入先 Program Files\\N225BrokerBridge
    """
    locations: list[Path] = []
    try:
        import winreg
        for hive, view in (
            ...
            try:
                key = winreg.OpenKey(
                    hive,
                    rf"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{BRIDGE_INSTALLER_APP_ID}_is1",
                    0, winreg.KEY_READ | view,
                )
                with key:
                    install_dir, _ = winreg.QueryValueEx(key, "InstallLocation")
                if install_dir:
                    locations.append(Path(install_dir) / BRIDGE_EXE_NAME)
            except OSError:
                pass
    except ImportError:
        pass
    for env in ("ProgramFiles", "ProgramW6432"):
        ...
        if pf:
            locations.append(Path(pf) / "N225BrokerBridge" / BRIDGE_EXE_NAME)
    return locations


def _resolve_bridge_exe() -> Path:
    """Bridge exe を開発環境と配布環境の両方で見つける。
    開発環境 (リポ直下): N225BrokerBridge/src/N225BrokerBridge.UI/bin/{Debug,Release}/... のビルド出力
    配布環境: setup.exe で導入済みのブリッジ（レジストリ InstallLocation / Program Files）
    ※配布パッケージにはブリッジのソースを同梱しない（setup.exe のみ・2026-07-14）。
    """
    candidates = [
        SCRIPT_DIR / "N225BrokerBridge" / "src" / "N225BrokerBridge.UI" / "bin" / "Debug"
            / "net8.0-windows" / BRIDGE_EXE_NAME,
        SCRIPT_DIR / "N225BrokerBridge" / "src" / "N225BrokerBridge.UI" / "bin" / "Release"
            / "net8.0-windows" / BRIDGE_EXE_NAME,
    ]
    candidates += _installed_bridge_locations()
    for c in candidates:
        if c.exists():
            return c
    return candidates[-1]  # 見つからない場合は導入先想定パスを返す (エラー文言で使う)
...
class SimulatorTestDashboard:
    ...
    def on_start_bridge(self):
        ...
        if not self.bridge_exe.exists():
            # 起動のたびに再解決（ダッシュボード起動後に setup.exe を実行したケースを拾う）
