...
> **ZIP 版（ポータブル）もあります**：展開先を自分で決めたい人向けに、同じ中身の ZIP も Release に置いています。展開して `N225BrokerBridge-Setup-*.exe`（ブリッジ）→ `起動_シミュレーション.exe` の順に実行してください。GitHub からソースだけを clone した場合、exe 類はリポジトリに含まれません（Release から取得）。
