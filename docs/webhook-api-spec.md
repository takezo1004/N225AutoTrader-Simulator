...
### 3.2-b 銘柄（ticker）の扱い ★現状は簡略化

**発注する銘柄を決めるのはブリッジ側**（メイン画面で選択している銘柄）。受信した `ticker` は
記録に残るのみで、**発注銘柄の判断には使わない**。

日経225先物には ラージ／ミニ／マイクロ があり、この作りにより
「**判断はミニのデータ・発注はマイクロ**」といった運用ができる。送信元が銘柄を指定しなくても、
ブリッジ側で選択している銘柄で約定する。

**本来は「送信元（ローカル版／TradingView 版）から送られた銘柄でブリッジが動作する」形が正**だが、
現状は上記のとおり簡略化している。将来、送信元が銘柄を指定する方式へ改める。
